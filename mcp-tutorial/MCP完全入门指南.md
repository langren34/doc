# MCP 完全入门指南：从协议原理到 OpenClaw 实战

> 目标读者：想系统理解 **Model Context Protocol（MCP）** 的初学者与开发者。
> 本文基于 [MCP 中文站](https://mcpcn.com/docs/introduction/) 与 [Claude Code 官方中文文档](https://code.claude.com/docs/zh-CN/agents) 整理，涵盖协议基础、三大原语、传输机制、实战开发、OpenClaw 中 MCP 集成、Channels 进阶与安全调试。
...
- [8. 在 OpenClaw 中使用 MCP](#8-在-openclaw-中使用-mcp)
...
OpenClaw 作为个人 AI 助手环境，通过 MCP 把模型连接到外部工具。Claude Code 是 Anthropic 推出的编码 Agent，同样通过 MCP 把 Claude 连接到 [GitHub、数据库、Slack、浏览器、监控系统等](https://code.claude.com/docs/zh-CN/mcp) [官方：code.claude.com/mcp]。例如：
...
![MCP 架构图](images/mcp-architecture.png)
...
![MCP 三大原语](images/mcp-primitives.png)
...
## 6. 传输机制：stdio、SSE、HTTP、WebSocket

MCP 的架构分为 **协议层** 和 **传输层** [官方：mcpcn.com/architecture]。

- **协议层（Protocol）**：处理消息帧、请求/响应链接、高级通信模式。所有 MCP 消息都封装在 JSON-RPC 2.0 中。
- **传输层（Transport）**：负责 Client 与 Server 之间的字节流传输。只要两端都实现同一传输，MCP 协议就可以跑在上面。

这种解耦意味着你可以先写好一个 MCP Server，然后按部署场景切换传输方式，而业务逻辑几乎不变。

![MCP 传输层](images/mcp-transports.png)
*图 3：协议层与传输层解耦 — 同一套 MCP 协议可以跑在 stdio、SSE、HTTP、WebSocket 等多种传输之上 [官方：mcpcn.com/architecture]*

### 6.1 stdio（标准输入输出）

**适用场景**：本地 Server，作为 Host 的子进程运行。

- Host 启动 Server 进程（如 `python server.py`）。
- 通过 `stdin` 发送 JSON-RPC 消息，通过 `stdout` 接收响应。
- Server 的 `stderr` 通常用于日志输出，不混入协议数据。
- 这是 Claude Code / OpenClaw 本地 MCP Server 最常用的方式 [官方：code.claude.com/mcp]。

优点：

- 天然进程隔离，崩溃不影响 Host。
- 无网络依赖，适合本地工具（文件系统、数据库、Shell）。
- 权限由操作系统进程控制，简单直接。

### 6.2 SSE（Server-Sent Events）

**适用场景**：远程 Server，需要服务器向客户端推送事件。

- 基于 HTTP，使用 SSE 建立从 Server 到 Client 的单向推送通道。
- Client 通过单独的 HTTP POST 向 Server 发送请求。
- 适合监控、通知、流式结果等场景。

优点：

- 兼容现有 HTTP 基础设施（负载均衡、CDN、防火墙）。
- 服务器可以主动向客户端推送消息。

限制：

- 本质上仍是 HTTP 请求/响应，实时性不如 WebSocket。
- 需要单独处理 POST 和 SSE 两条通道。

### 6.3 HTTP / Streamable HTTP

**适用场景**：纯 Web 环境，不需要长连接。

- 每个 MCP 消息都是一次 HTTP 请求/响应。
- 适合 Serverless、函数计算、REST 友好架构。
- 最新 MCP 规范也支持 **Streamable HTTP**，在普通 HTTP 之上实现流式响应 [官方：mcpcn.com/architecture]。
...
- 每次请求都有 TCP/TLS 握手开销。
- 服务器无法主动推送（除非配合轮询或 SSE）。

### 6.4 WebSocket

**适用场景**：实时双向通信，低延迟交互。

- 建立一次握手后，Client 和 Server 可以在全双工通道上随时发送消息。
- 适合聊天机器人、实时协作、游戏等场景。
...
| 场景 | 推荐传输 | 原因 |
|------|----------|------|
| 本地 CLI / IDE 插件 | stdio | 简单、进程隔离、无网络 |
| 浏览器内 Agent | HTTP / SSE | 兼容 Web 栈、穿越防火墙 |
| 远程实时服务 | WebSocket / SSE | 低延迟或服务器推送 |
| Serverless / 云函数 | HTTP | 无状态、易扩展 |

> 在 OpenClaw 中，最常见的 MCP Server 是 **stdio** 本地 Server；如果你要暴露服务给远程客户端，再考虑 HTTP / SSE / WebSocket [本机验证]。
...
## 8. 在 OpenClaw 中使用 MCP

OpenClaw 通过 MCP 把模型连接到外部工具。配置方式参考 Claude Code，有三种：
...
OpenClaw 的配置范围与 Claude Code 类似，均支持三个级别 [官方：code.claude.com/mcp]：
...
把我们刚才写的 `server.py` 添加到 OpenClaw：
...
OpenClaw 支持类似能力 [官方：code.claude.com/mcp]：

- **自动重新连接**：HTTP/SSE 服务器断开时，会以指数退避重试最多 5 次。
- **动态工具更新**：Server 发送 `list_changed` 通知时，OpenClaw 自动刷新可用工具列表。

---

## 9. 用 Agent SDK 编程调用 MCP

Claude Code 终端之外，你也可以在自己的 Python/TypeScript 应用里用 **Claude Agent SDK** 调用 MCP Server。OpenClaw 同样支持基于 Agent SDK 或内置机制集成 MCP Server。
...
async def main():
    ...
    async for message in query(
        prompt="Use the docs MCP server to explain what hooks are in OpenClaw",
        options=options,
    ):
...
> 这个示例来自 Claude Code Agent SDK 文档 [官方：code.claude.com/agent-sdk/mcp]，在 OpenClaw 中可直接复用相同模式。
...
标准 MCP 是 **拉模式**：模型主动调用 Server 的工具。但很多时候我们需要 **推模式**：外部事件（CI 失败、监控警报、Telegram 消息）主动进入 OpenClaw 会话。

**Channels** 就是 Claude Code 对 MCP 推送能力的扩展 [官方：code.claude.com/channels]。

### 10.1 什么是 Channel

Channel 是一个 MCP Server，它把事件推送到运行中的 OpenClaw / Claude Code 会话，让模型在你不在终端时也能对外部事件做出反应 [官方：code.claude.com/channels]。
...
![Channel 架构](images/channel-architecture.svg)
*图 4：Channel 架构 — 外部系统（Telegram / Discord / Webhook）通过本地 Channel Server 把事件推送到 OpenClaw / Claude Code 会话 [官方：code.claude.com/channels-reference]*

OpenClaw / Claude Code 把 Channel Server 作为子进程生成，通过 stdio 通信。Channel Server 在本地监听 HTTP 或轮询聊天平台 API，当事件发生时推送 `notifications/claude/channel` 通知 [官方：code.claude.com/channels-reference]。
...
启动 OpenClaw / Claude Code 时启用 channel：
...
![权限中继](images/channel-permission-relay.svg)
*图 5：Channel 权限中继 — OpenClaw / Claude Code 把权限提示转发到 Channel Server，Server 发送到聊天应用，用户回复后 Server 把判决传回 OpenClaw / Claude Code [官方：code.claude.com/channels-reference]*
...
OpenClaw 与 Claude Code 都对 MCP 工具有多层控制 [官方：code.claude.com/agent-sdk/mcp]：
...
在 OpenClaw 中 [官方：code.claude.com/debug-your-config]：
...
来自 Claude Code 官方调试文档，OpenClaw 同样适用 [官方：code.claude.com/debug-your-config]：
...
Claude Code 文档给出了一张功能加载流程图，OpenClaw 的加载逻辑类似 [官方：code.claude.com/features-overview]：

![上下文加载](images/context-loading.svg)
*图 6：Claude Code / OpenClaw 功能加载流程 — CLAUDE.md 在每个请求中加载；MCP 工具名称启动时加载，完整架构延迟到使用；Skills 描述启动时加载，完整内容使用时加载；Hooks 外部运行 [官方：code.claude.com/features-overview]*
...
- **进阶者**：在 OpenClaw 中接入真实 Server（GitHub、PostgreSQL），体验自然语言驱动外部工具（第 8 章）。
...
- [Claude Code 文档 — MCP](https://code.claude.com/docs/zh-CN/mcp)（OpenClaw 兼容）
- [Claude Code 文档 — Channels](https://code.claude.com/docs/zh-CN/channels)（OpenClaw 兼容）
- [Claude Code 文档 — 调试配置](https://code.claude.com/docs/zh-CN/debug-your-config)（OpenClaw 兼容）

---

> 版本说明：本文基于 2026-07-23 抓取的 MCP 中文站与 Claude Code 中文文档整理。MCP 协议与 OpenClaw / Claude Code 都在快速迭代，建议以官方最新文档为准。
