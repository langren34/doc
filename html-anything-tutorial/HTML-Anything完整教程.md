...
| **OpenClaw** | `openclaw` | argv-message | `npm i -g openclaw` |
...
> ❗ **快捷键注意**：图里按钮 label 显示 `⌘+↵` 是**产品图标**（macOS 习惯）。**Windows / Linux 实际快捷键是 `Ctrl+Enter`**（不是 `⌘+Enter`）——如果你按 `⌘`（Mac） 或 `Ctrl`（Win/Linux） 按不动，试试点右侧的 `Convert to HTML` 文字按钮本身。
...
日志关键行（截图里是 Claude Code 跑的样例——你机器上用什么 agent，下面会显示对应名字，比如 OpenClaw 会显示 `openclaw` / Kimi 显示 `kimi` 等）：
