...
![Admin → Gitaly Servers](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-2.1-gitaly-servers.png)
...
![Admin 仪表板的安全警告条幅](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-4.5-admin-overview.png)
...
![Public 项目 skills-repo](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.1.5b-skills-repo-visibility.png)
*图：`root/skills-repo`（Public）空仓库主页——任何匿名访问者能看到 main 分支和 README*

![Private 项目 dev-code-sample](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.1.5c-dev-sample-visibility.png)
*图：`root/dev-code-sample`（Private）空仓库主页——未登录访问会被重定向到登录页*

---

### 5.1.6 创建用户、组、绑定项目（[本机验证]）

#### 5.1.6.1 创建用户（Web UI）

**路径 1**：`Menu` → **Admin** → **New User**（或直接访问 `/admin/users/new`）

| 步骤 | 操作 |
|------|------|
| 1. 填写基本信息 | Name / Username / Email |
| 2. 选择访问级别 | **Regular**（普通用户）/ Admin（管理员） |
| 3. 点击 **Create user** | 自动发送激活邮件（**首次重置密码邮件会含 URL**） |
...
*图：Admin → New User 页面，alice 的 Name / Username / Email 均已填写（[本机验证]）。密码字段是「自动生成重置链接」二选一选项——本机部署默认不发邮件，用户创建后管理员需用 rails runner 或重新设置密码才能登录*

**本教程创建了 3 个普通用户**（均选择 **Regular** 访问级别）：

| 用户 | Username | Email（示例） | 本教程测试密码 |
|------|----------|---------------|----------------|
| Alice | `alice` | `alice@example.com` | `TestUser2026!` |
| Bob | `bob` | `bob@example.com` | `TestUser2026!` |
| Carol | `carol` | `carol@example.com` | `TestUser2026!` |

#### 5.1.6.2 创建组（Web UI）

**步骤 1**：进入入口页（GitLab 19 改为两步：选方式 → 填表单）

**路径**：`Menu` → **Groups** → **New group**（或直接访问 `/groups/new`）

![创建组入口](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.1.6b-create-group-form.png)
*图：`/groups/new` 入口页（[本机验证]）。GitLab 19 把「创建组」拆成两步：先选「创建群组」或「导入群组」卡片，再进下一步表单。点 **创建群组** 进入表单*
...
#### 5.1.6.3 给组添加成员（Web UI）

**路径**：组页面 → **Manage** → **Members**（或直接访问 `/groups/<group-path>/-/group_members`）

![组成员列表](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.1.6c-group-members-list.png)
*图：Team Alpha 组的「群组成员」页（[本机验证]）。当前 4 个成员：root (Owner) + alice (Developer) + bob (Maintainer) + carol (Reporter)。右上角 **邀请群组**/**邀请成员** 按钮可见（Maintainer 以上才有）*

**邀请成员表单字段**：

- **GitLab member or Email**：输入用户名或邮箱
- **Choose a role permission**：选择角色（Guest/Reporter/Developer/Maintainer/Owner）
- **Access expiration date**（可选）：访问权限到期时间

#### 5.1.6.4 绑定项目到组（Web UI，GitLab 19 推荐方式）

GitLab 19 把「Share with group」从项目设置移到 **Members 页面**——项目 → **Manage** → **Members**。

![项目 Members 页 — 成员标签](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.1.6d-invite-member-skills.png)
*图：Skills Repository 项目 → Members 页（root 视角，[本机验证]）。**「成员」标签页**：4 个项目成员 + 3 个右上按钮（**从一个项目导入**/**邀请群组**/**邀请成员**）。成员列表的「来源」列清晰展示 **谁是通过哪个组继承的**——例如 `Alice Wang` 来源是「Team Beta 群组」*

![项目 Members 页 — 群组标签](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.1.6e-share-with-group.png)
*图：Skills Repository 项目 → Members 页，切到「群组」标签（[本机验证]）。**「群组」标签页** = 「Share with group」功能的新位置。可见 2 个组链接：Team Alpha (Maintainer) + Team Beta (Developer)。每行可改角色/过期时间/移除。**右上「邀请群组」** 按钮是给项目加新共享组的入口*

**步骤**：

1. 项目页 → 左侧 **Manage** → **Members**
2. 切到 **群组** 标签
3. 右上 **邀请群组** 按钮
4. 选组 + 选角色 + 可选过期时间 → 邀请

> 💡 绑定后，**组内所有成员自动获得项目权限**（按 min(组员角色, link 角色) 计算）。后续组里加/删成员时无需动项目。

#### 5.1.6.5 直接给项目添加成员（不走组，Web UI）

**路径**：项目 → **Manage** → **Members** → **邀请成员**（root 视角下才看到这个按钮）

**邀请表单**：

- **GitLab member or Email**：用户名或邮箱
- **Choose a role permission**：角色（Guest/Reporter/Developer/Maintainer/Owner）
- **Access expiration date**（可选）

**本教程的 3 个项目成员角色**：

| 用户 | skills-repo 角色 | dev-code-sample 角色 |
|------|------------------|----------------------|
| alice | Developer (30) | Developer (30) |
| bob | Maintainer (40) | Maintainer (40) |
| carol | Reporter (20) | Reporter (20) |

> ⚠️ 直接成员 + 组继承的权限**取最高**（详见 §5.1.7）。

#### 5.1.6.6 命令行方式（API）

```bash
# 用 admin token 创建用户（避免邮件依赖）
curl -X POST http://localhost/api/v4/users \
  -H "PRIVATE-TOKEN: glpat-***" \
  -H "Content-Type: application/json" \
  -d '{"username":"alice","email":"alice@example.com","name":"Alice Wang","password":"TestUser2026!","skip_confirmation":true}'

# 给项目加成员
curl -X POST http://localhost/api/v4/projects/4/members \
  -H "PRIVATE-TOKEN: glpat-***" \
  -H "Content-Type: application/json" \
  -d '{"user_id":"4","access_level":30}'   # 30 = Developer

# 把项目分享给组
curl -X POST http://localhost/api/v4/projects/4/share \
  -H "PRIVATE-TOKEN: glpat-***" \
  -H "Content-Type: application/json" \
  -d '{"group_id":"13","group_access":40}'  # 40 = Maintainer
```
...
| 用户 | skills-repo 项目角色 | dev-code-sample 项目角色 | 来源（是否走组） |
|------|---------------------|---------------------------|---------------------|
| alice | Developer (30) | Developer (30) | 项目直接成员 |
| bob | Maintainer (40) | Maintainer (40) | 项目直接成员 |
| carol | Reporter (20) | Reporter (20) | 项目直接成员 |

另外，项目还共享给组（允许组内成员后续自动获得权限）：

| 组 | shared to skills-repo | shared to dev-code-sample |
|----|------------------------|---------------------------|
| Team Alpha | Maintainer (40) | Developer (30) |
| Team Beta | Developer (30) | — |

#### 组与项目权限叠加规则

- 项目直接授予权限（Project Member）和通过组继承的权限（Group Member）**取两者的最大值**
- 例：若 bob 在 team-alpha 是 40/Maintainer，且 team-alpha shared to skills-repo 是 link=40，则 bob 在 skills-repo = min(40, 40) = **40/Maintainer**
- 例：若 carol 在 team-alpha 是 20/Reporter，team-alpha link=40 → carol = min(20, 40) = **20/Reporter**（组员角色是上限）

#### 不同角色访问同一项目的实际效果（[本机验证]）

以下 3 张图是 **alice / bob / carol 3 个用户实际登录后** 访问 `skills-repo` 项目 → **Manage → Members** 页面的截图。注意右上角「邀请成员」/「邀请群组」按钮的差异：

**alice (Developer, 30)** —— **看不到**邀请按钮：

![alice 视角](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.1.7a-alice-developer-skills.png)
*图：alice (Developer) 登录后看 skills-repo → Members 页（[本机验证]）。**页面提示**：`Members can be added by project Maintainers or Owners`。**右上角没有「邀请成员」按钮**。alice 能看到所有成员（自己显示绿色 "It's you" 徽章），但**不能邀请新成员**——Developer 默认权限限制*

**bob (Maintainer, 40)** —— **有**邀请按钮：

![bob 视角](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.1.7b-bob-maintainer-skills.png)
*图：bob (Maintainer) 登录后看 skills-repo → Members 页（[本机验证]）。**右上角完整 3 个按钮**：Import from a project / Invite group / **Invite member**（蓝）。bob 可以邀请新成员、邀请新组、修改任何成员的角色/过期时间。表里显示 bob 自己也是成员（Maintainer）*

**carol (Reporter, 20)** —— **看不到**邀请按钮：

![carol 视角](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.1.7c-carol-reporter-skills.png)
*图：carol (Reporter) 登录后看 skills-repo → Members 页（[本机验证]）。**同样没有「邀请成员」按钮**。carol 能看 Members 列表（自己显示 "It's you"），但**只能看不能改**。Reporter 不能邀请成员，也不能修改任何成员的角色*

> 🔑 **核心规律**：GitLab UI 里「邀请成员」按钮只对 **Maintainer + Owner** 可见——Developer / Reporter / Guest 都看不到。这与 API 的 `:admin_project_member` 权限一致。

#### 跨项目权限隔离示例（carol 访问 Private 项目）

carol 是 skills-repo 的 Reporter，但 **不是 dev-code-sample 的成员**（也没通过任何组继承），且 dev-code-sample 是 **Private 项目**：

![carol 无权访问](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.1.7d-carol-forbidden-dev-sample.png)
*图：carol 登录后访问 `/root/dev-code-sample` 看到的页面（[本机验证]）。**显示 404 而不是 403**——GitLab 故意用 404 而不是 403 来避免泄露项目存在性（安全最佳实践）*

> 💡 如果是**未登录用户**访问 Private 项目，行为不同——直接重定向到登录页：

![未登录访问 Private](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.1.7e-anonymous-private.png)
*图：未登录状态访问 `/root/dev-code-sample`（[本机验证]）。被重定向到 `/users/sign_in`。已登录用户访问 Private 项目无权限时才看到上图的 404（区别对待避免信息泄露）*
...
| 工具 | 一句话 | 详细路径 | 图 |
|------|--------|---------|-----|
| **Wiki** | 项目自带 Markdown 文档站，支持版本历史 + Sidebar | `项目菜单` → **Wiki** → **Create your first page** | ![Wiki](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.5-wiki.png) |
| **Snippet** | 个人/团队代码片段分享（多文件 + 语法高亮） | `+` → **New snippet**，选 Visibility | ![Snippet](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.7-snippet-new.png) |
| **Release** | 基于 Git tag 的版本发布（带 Markdown notes + 二进制） | `Deployments` → **Releases** → **New release** | ![Release](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.8-release-new.png) |
| **Pages** | 静态网站托管（根目录 `.gitlab-ci.yml` 定义 `pages` job） | §5.9 + §4.7（server 端 `pages_external_url`） | — |
| **Registry** | 项目内置 Container / Package Registry | §5.10 + §4.6 | ![Packages](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.10-packages.png) |
| **glab** | GitLab 官方 CLI（issue / mr / ci / api / repo） | §5.11 | ![glab](https://cdn.jsdelivr.net/gh/langren34/doc@main/gitlab-tutorial/images/gitlab-5.11-glab-install.png) |
